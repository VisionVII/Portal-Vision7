...
- [x] Acessos — **fix de segurança aplicado em 2026-08-30 e redesign visual concluído.** Auditoria encontrou escalada de privilégio confirmada: qualquer `admin` conseguia convidar alguém (ou registar-se a si próprio) como `super_admin`, porque `send-invite-code` nunca verificava se o papel pedido era igual ou inferior ao de quem convida — só verificava que quem convida é admin/super_admin, não o papel do convidado. Corrigido na origem (`send-invite-code` só deixa admins normais convidar para papéis operacionais; admin/super_admin passam a exigir que quem convida seja super_admin) e reforçado nos dois caminhos de resgate do convite (`activate-invite`, `assign-invite-role` — este último é o realmente usado pelo registo público em `UserLogin.tsx`), que voltam a verificar o papel actual de quem convidou antes de atribuir um papel elevado. **Estas são Edge Functions — o fix só protege produção depois de `supabase functions deploy`, não há deploy automático via CI para functions.** Revisão adversarial ao fix dos convites encontrou um caminho de exploração bem mais directo e ainda por fechar: a política RLS `"Only admins can manage roles"` em `user_roles` era `FOR ALL` (cobre SELECT+INSERT+UPDATE+DELETE) só a verificar `has_role(admin)` — sem olhar para a linha a escrever. Ou seja, qualquer `admin` conseguia fazer `POST /rest/v1/user_roles {"role":"super_admin",...}` directamente, sem passar pelo fluxo de convites nem por nenhuma RPC. Corrigido em `20260830120000_fix_user_roles_rls_privilege_escalation.sql` — a política deixou de permitir escrita directa a `authenticated` (toda a escrita legítima já passa por RPCs `SECURITY DEFINER`, confirmado que nenhuma escrita ao vivo depende do acesso directo). A mesma revisão encontrou ainda que `has_role()` nunca verificava `is_active` — "Desativar membro" nunca tinha efeito real a nível de RLS/RPC, só escondia o UI (`AuthContext.tsx` filtra `is_active` no frontend, mas o backend não). Corrigido em `20260830120100_fix_has_role_ignores_is_active.sql`. **Ambas as migrations estão escritas mas por aplicar — precisam de `supabase db push` (ou equivalente) para protegerem produção; não há deploy automático de migrations via CI.** Redesign visual concluído: resolvido o desfasamento da validade do convite (agora selecionável entre 24h, 48h [padrão] e 7 dias, devidamente propagado para a base de dados e para a Edge Function `send-invite-code`), introduzidos novos stat cards semânticos (Ativos em tom esmeralda, Pendentes em âmbar, Inativos em slate), abas refinadas com badges dinâmicos, avatares estilizados na tabela de equipa e badges de estado dinâmicos na lista de convites.
- [x] Developer — Redesign visual concluído e reforço de segurança aplicado. Separadores modernizados com ícones e preservação dos atributos `data-tour` (`developer-tab-diagnostics`, `developer-tab-vault`, `developer-content-diagnostics`, `developer-content-vault`). Diagnósticos & Métricas atualizado com cartões de infraestrutura semânticos (Base de Dados, Servidor, Sessão, Conectividade), métricas consolidadas (Total de Posts, Categorias, Sessões), persistência de notas técnicas com feedback de estado e ranking dos 5 artigos mais visualizados. Credential Vault reestruturado com stat cards semânticos (Total, Ativas [esmeralda], A expirar [âmbar]), substituição da eliminação/revogação direta por diálogo de confirmação seguro (`AlertDialog`), cópia rápida do nome da credencial e formulário moderno com alternador de visibilidade de segredo.
- [x] Configurações — Redesign visual concluído. `SettingsView.tsx` reestruturado com barra de separadores modernos com ícones (Geral & Identidade, Assistente IA, Monetização, Tutorial & Guia) e lazy loading para cada painel. `tour-types.ts` atualizado com `activateSelector` nos 3 passos de configurações (`settings-tab-general`, `settings-tab-ai`, `settings-tab-tutorial`) para sincronizar o tutorial interativo com os novos separadores — exatamente como foi feito para Automações, CRM e Acessos. `SiteSettingsManager.tsx` modernizado com layout de identidade limpo (preview circular do logótipo, botão de upload semântico), formulário de nome do portal com feedback de estado ("Guardado" em esmeralda), cartão SMTP com badge de estado contextual (Configurado/Pendente), nota inline de segurança a remeter para o Credential Vault e botão com feedback. `MonetizationManager.tsx` polido com badges semânticos de estado (Ativa/Inativa) nos 3 cartões, badges interativos de slot de anúncio e remoção de funcionalidades premium com transições suaves. `AISettingsPanel.tsx` atualizado com badge de estado (Ativo/Inativo em violeta), seletor de modelo enriquecido com ícones e descrições (Haiku = Zap/azul, Sonnet = Sparkles/violeta), badges de competências com cor activa e indicador de gravação inline.
